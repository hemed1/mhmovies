...
// import 'package:mhnotes/pages/SettingPage.dart';
...
class _HomePageState<T> extends State<HomePage>   //implements ListView_Extend
{
  ...
  // final Color buttonsIconColor =  Colors.black;  //Theme.of(_context).colorScheme.primary;
  // final Color buttonsForegroundColor = Colors.blue;       //Theme.of(context).colorScheme.primary;
  // late final Color buttonsRowBackgroundColor = Theme.of(_context).colorScheme.primaryContainer.withOpacity(0.4);    // Row of buttons background color
  ...
  {
    super.initState();
    // NotificationHandle.onNotificationDismiss = this.onNotificationDismiss;
    ///*await*/ setFilterToList(currentFilter, false, null);  // Because 'context' not yet init
  }
  ...
  Widget build(BuildContext context)
  {
    ...
    {
      isFirstTime = false;
      // TODO:  this.readAllRecords(true, true);
    ...
    screenHeight = size.height - 470.0;  // AppBar width, down buttons width
    screenWidth = size.width - 147;


    ButtonStyle buttonStyle = ButtonStyle(foregroundColor: MaterialStateProperty.resolveWith(getColorForButtons));

    widgetHomePage = WidgetHomePage<T, DataProvider<T>>(
                          title: this.title,
                          mainList: this.dataProvider.objectsList as List<T>,
                          // clientColorScheme: Theme.of(_context).colorScheme,     //Globals.applicationColorScheme,    //Theme.of(_context).colorScheme,
               ...
               Column(
                 ...
                 [
                    Row(
                      ...
                      children:
                      [
                        Text('משימות ופתקים', textAlign: TextAlign.right,
                            style: TextStyle(
                                      fontSize: 23.0,
        ...
        listTile = ListTile(
            ...
            onTap: () async
            {
              ...
              await Navigator.push(_context, MaterialPageRoute(
                                                ...
                                                {
                                                  return DatabaseHandle(2, this._context, this.dataProvider).widgetToShow;     //DatabaseHandlePage();
                                                }));
            });
        DatabasesListTiles.add(listTile);
    }
  ...
  Widget CreateListItemTileFunc(BuildContext context, dynamic objectInstantFirst)
  {
    ...
    styleRichText = TextStyle(color: Colors.black87,
        ...
        fontSize: 16.0,
        height: 1.0,
        decoration: (movieInstant.StatusID == NoteStatusEn.Completed.value) ? TextDecoration.lineThrough : TextDecoration.none,
        letterSpacing: 0.0,
        wordSpacing: 0.08);

    // Main image
    final Image image = Globals.getImageByListType(movieInstant);
  ...
  //   }
  //
  //   movieInstant = object as Movie;
  //
  //   isShowDetails = false;
  //
  //   // Main image
  //   Image image = Globals.getImageByListType(movieInstant);
  //
  //   // leading image
  //
  //   leadingImageWidget = (!widgetHomePage!.isSelectMode  /*widgetHomePage.widgetHomePageState!.isSelectMode*/)
  ...
  //       color: Theme.of(context).colorScheme.primary,
  //       hoverColor: Colors.red,
  //       icon: Icon(Icons.delete, size: 20.0, color: Theme.of(context).colorScheme.primary),
  //       onPressed: () async
  //       {
  //         List<Movie> selectedItems = [ movieInstant ];
  //         List<int> items = selectedItems.map((e) => e.MovieID).toList();
  //
  //         double scrollPosition = this.scrollPosition;
  //         int scrollerIndex = this.scrollerIndex;
  //         await this.dataProvider.deleteRecord(selectedItems, "MovieID", true);
  //       });
  //
  //
  //   final String description = movieInstant.Description.trim();
  //
  //   // Title widget
  //   titleWidget = Text(movieInstant.Title,
  //       overflow: TextOverflow.ellipsis,
  //       style: TextStyle(
  //           color: Theme.of(context).colorScheme.primary,
  //           fontWeight: FontWeight.w600,
  ...
  //       // (movieInstant.StatusID == NoteStatusEn.Completed.value || (movieInstant.StatusID == NoteStatusEn.PartialCompleted.value)
  //       // ? TextDecoration.lineThrough
  //       // : TextDecoration.none)),
  //       maxLines: (description.isEmpty ? 2 : 1),
  //       textDirection: TextDirection.rtl,
  ...
  //           [
  //             // titleWidget,
  //             SizedBox(width: screenWidth-151  /*138*/, child: titleWidget),
  //
  //             deleteWidget
  //           ]);
  //
  //
  //   // Description field
  //   if (description.isNotEmpty || movieInstant.Year.isNotEmpty)
  //   {
  //     subTitleWidget =
  //         Column(
  //             crossAxisAlignment: CrossAxisAlignment.start,
  ...
  //                     textAlign: TextAlign.right),
  //
  //               if (movieInstant.Year.isNotEmpty)
  //                 Text(movieInstant.Year,
  //                     style: const TextStyle(color: Colors.blue, fontWeight: FontWeight.w500, fontSize: 12.5),
  //                     maxLines: 1,
  //                     textDirection: TextDirection.rtl,
  //                     textAlign: TextAlign.right),
  //             ]);
  ...
  Map<String, dynamic> _getExportMenuItem()
  {
    Map<String, dynamic> items = {};
    List<MapEntry<String, String>> mapEntriesList= [];
    MapEntry<String, String> mapItem;



    mapItem = const MapEntry('1', 'העתקה מלאה של כל הקובץ');
    mapEntriesList.add(mapItem);

    mapItem = const MapEntry('2', 'ייצוא לקובץ Json');
    mapEntriesList.add(mapItem);

    mapItem = const MapEntry('3', 'ייבוא מקובץ Json');
    mapEntriesList.add(mapItem);

    mapItem = const MapEntry('4', 'העתקה ממסד-נתונים מקומי למסד בענן');
    mapEntriesList.add(mapItem);

    mapItem = const MapEntry('5', 'העתקה ממסד-נתונים מהענן למסד מקומי');
    mapEntriesList.add(mapItem);

    items.addEntries(mapEntriesList);
  ...
  Future<List<Map<String, dynamic>>> sortMap(List<Map<String, dynamic>> mapValues) async
  {
    ...
    {
      final Map<String, dynamic> map = {'ID': item.value as int, 'Description': item.label as String};
      resultMaps.add(map);
    }
