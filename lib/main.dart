...
class MyApp extends StatelessWidget   //StatefulWidget  //
{
  ...
  static ColorScheme appColorScheme = Globals.applicationColorScheme;      //ColorScheme.fromSeed(seedColor: applicationMainColor);
...
// {
      ...
      MaterialApp(
          ...
          home:
            Directionality(
                ...
                child:
                  MyHomePage(title: title, version: this.version),
        ...
        MaterialPageRoute(
            builder: (_) =>
                ...
                Directionality(
                  textDirection: TextDirection.rtl,
                  child:
                  SafeArea(child: MyHomePage(title: 'משימות ופתקים', version: this.version)),
                )
        ));
  ...
  Route<dynamic>? onGenerateRoute(RouteSettings settings)
  {
    ...
    {
      return MaterialPageRoute(builder: (_) => SafeArea(child: MyHomePage(title: 'משימות ופתקים', version: this.version)));
    }
...
{
  MyHomePage({super.key, required this.title, required this.version});
...
class _MyHomePageState extends State<MyHomePage>   //with WidgetsBindingObserver
{
  ...
  Widget build(BuildContext context)
  {
    ...
    if (isFirstTime && !Globals.isMainInitApplicationLoaded && !MainGlobals.g_isInitApplicationDone)
    {
      isFirstTime = false;
      MainGlobals.g_isInitApplicationDone = true;
      Globals.isMainInitApplicationLoaded = true;
  ...
  Future<void> initApplication() async
  {
    ...
    Globals.isMainInitApplicationLoaded = false;
  ...
  {
    final Color applicationMainColor = Globals.translateMainColor();
    final ColorScheme colorScheme = Globals.applicationColorScheme;

    MyApp.appColorScheme = colorScheme;
    MyApp.applicationMainColor = applicationMainColor;

    if (Globals.keepOld != MainGlobals.g_colorsPattern)
    {
      setState(()
      {
        MyApp.appColorScheme = colorScheme;
    ...
    //   {
    //     MyApp.applicationMainColor = applicationMainColor;
    //     MyApp.appColorScheme = ColorScheme.fromSeed(seedColor: applicationMainColor);
    //   });
    // }
